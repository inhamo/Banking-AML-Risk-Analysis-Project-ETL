	...
	BEGIN TRY
        ...
        *********************************************/
		...
		SELECT 
			...
			UPPER(TRIM(trading_name))                     AS company_trading_name,
        ...
        *********************************************/
		...
		-- Check if stored procedure exists and execute it
		IF EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'pcd_insert_into_customer_normalized_tables' AND schema_id = SCHEMA_ID('silver'))
		BEGIN
			EXEC silver.pcd_insert_into_customer_normalized_tables;
			PRINT 'Stored procedure executed successfully';
		END
		ELSE
		BEGIN
			PRINT 'Stored procedure silver.pcd_insert_into_customer_normalized_tables does not exist';
		END
        ...
        *********************************************/
		...
		-- Drop temp table to clear up space 
		IF OBJECT_ID('tempdb..#stg_customers') IS NOT NULL
			DROP TABLE #stg_customers;
